...
import { setFlash } from "./setFlash.ts";
...
export function redirect(
  ...
  if (typeof statusOrLocationOrMessage === "number") {
    ...
    svelteKitRedirect(
      statusOrLocationOrMessage,
      locationOrMessage as string | URL,
    );
  }

  if (
    typeof statusOrLocationOrMessage === "string" ||
    statusOrLocationOrMessage instanceof URL
  ) {
