<script lang="ts">
  import { getFlashStore, addFlashMessage } from "$lib/client.js"

  const message = getFlashStore<App.PageData['flash']>()

  function change() {
    const msg = {status: 'error' as const, text: 'Updated on client from other component!'}
    message.update(m => m ? [...m, msg] : [msg])
  }
  ...
  async function submitForm(e : SubmitEvent) {
    ...
    message.update(m => addFlashMessage(m))
  }
</script>
...
<article>
  ...
  <form method="POST" id="">
    <button id="action-post">Submit</button>
  </form>
  ...
  <form method="POST" action="/test" on:submit|preventDefault={submitForm}>
    ...
    <button id="endpoint-client">Submit to /test client-side</button>
  </form>

  <form method="POST" action="/test">
    <button id="endpoint-server">Submit to /test by POST request</button>
  </form>
</article>
