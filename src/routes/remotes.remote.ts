...
import { redirect, setFlash } from "#lib/server/index.ts";
...
export const remoteFlashCommand = command(
  type("string > 0"),
...
export const remoteRedirect = form(
  flashFormSchema,
  async ({ name, useGoto }) => {
    const message =
      name + " posted at " + new Date().toTimeString().substring(0, 8);
